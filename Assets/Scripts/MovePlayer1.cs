...
public class MovePlayer1 : MonoBehaviour {
    ...
    void Update() { 
        ...
        Vector2 moveDir = new Vector2(Input.GetAxisRaw("P1_horizontal") * moveSpeed, rb2D.velocity.y);
        ...
        if (Input.GetKey(KeyCode.RightArrow) && Input.GetKey(KeyCode.Slash))
        {
            moveSpeed = 7;
            GetComponent<Animator>().SetBool("Running", true);
        }
        else if (Input.GetKey(KeyCode.LeftArrow) && Input.GetKey(KeyCode.Slash))
        {
            moveSpeed = 7;
            GetComponent<Animator>().SetBool("Running", true);
        }
